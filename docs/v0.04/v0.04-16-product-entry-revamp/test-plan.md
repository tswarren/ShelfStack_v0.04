...
| Service short form | `catalog_item_type: other`, `product_type: service`, minimal fields |
| Non-inventory short form | `product_type: non_inventory` |
...
| Ingram import (if covered) | Still creates valid products |
...
4. Add Item → Service: short form only
5. Product edit → change store category → subdepartment/display preview updates
6. Variant edit → selling price as dollars, no SKU field
7. Setup → Formats → MVP list present with correct kind/digital flags
...
3. Call resolver with fixture contexts (no DB required for pure checks, or use test fixtures)
4. Optionally smoke one Add Item integration example in test env
