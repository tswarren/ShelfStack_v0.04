...
module Setup
  class CategoryNodesController < BaseController
    ...
    def load_form_collections
      @parent_nodes = @category_scheme.category_nodes.active_records.where.not(id: @category_node&.id).order(:sort_order, :name)
    end

    def category_node_params
      params.require(:category_node).permit(:node_key, :name, :parent_id, :sort_order, :active)
    end
  end
end
